...
def CreateDataset(datasets_save_path, ratios, image_shape, masks_path, inputs_path, data_len, num_workers, seed, spatial_dims=3):
    if not data_len:
        data_len = len(glob.glob(inputs_path+"**/*[!output].tif", recursive=True))

    depth = image_shape[0]
    train_idx, test_idx, val_idx = util.DataSplit(ratios, None, seed, data_len, spatial_dims, depth)
